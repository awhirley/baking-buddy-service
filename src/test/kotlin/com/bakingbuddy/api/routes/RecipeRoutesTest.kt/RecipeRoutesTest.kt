...
private fun sampleRecipe(id: Uuid = Uuid.random()): Recipe = Recipe(
    id = id,
...
private fun sampleRecipeDetail(id: Uuid = Uuid.random()): RecipeDetail = RecipeDetail(
    id = id,
    name = "Sourdough Loaf",
    description = "Basic sourdough",
    recipeSource = null,
...
private fun sampleIngredient(id: Uuid = Uuid.random(), recipeId: Uuid = Uuid.random()): Ingredient = Ingredient(
    id = id,
    recipeId = recipeId,
    bestVersion = 1,
    notes = null,
...
private fun sampleInstruction(id: Uuid = Uuid.random(), recipeId: Uuid = Uuid.random()): Instruction = Instruction(
    id = id,
    recipeId = recipeId,
    bestVersion = 1,
    notes = null,
...
private fun validCreateRecipePayload() = CreateRecipePayload(
    name = "Sourdough Loaf",
    description = "Basic sourdough",
    recipeSource = null,
    tags = null,
...
class RecipeRoutesTest {
    ...
    @Test
    fun `GET recipe by id returns 200 when found`() = testApplication {
    ...
    @Test
    fun `GET recipe by id returns 404 when service returns null`() = testApplication {
        val service = mockk<RecipeService>()
        val id = Uuid.random()
    ...
    @Test
    fun `GET recipe by id returns 400 for a malformed uuid and never calls the service`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `GET recipes returns 200 with the service's list`() = testApplication {
        val service = mockk<RecipeService>()
    ...
    @Test
    fun `POST recipe with a valid payload returns 201 and calls the service`() = testApplication {
        val service = mockk<RecipeService>()
    ...
    @Test
    fun `POST recipe with malformed JSON returns 400 Invalid request body`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `POST recipe with a blank name returns 400 with a field error and never calls the service`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `POST recipe with no ingredients returns a field error for ingredients`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `POST recipe with no instructions returns a field error for instructions`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `POST recipe with a blank ingredient name reports an indexed field path`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `POST recipe with a blank ingredient amount reports an indexed field path`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `POST recipe with a blank instruction reports an indexed field path`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `POST recipe with a blank tag reports an indexed field path`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `POST recipe with null tags does not trigger a tags validation error`() = testApplication {
        val service = mockk<RecipeService>()
    ...
    @Test
    fun `POST recipe collects multiple field errors in a single response`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `PATCH recipe with a valid payload calls editRecipe with the parsed body`() = testApplication {
        ...
        val response = client.patch("/api/recipes/$id") {
            contentType(ContentType.Application.Json)
            setBody(payload)
        }
    ...
    @Test
    fun `PATCH recipe with a malformed uuid returns 400 and never calls the service`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)

        val response = client.patch("/api/recipes/not-a-uuid") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `PATCH recipe with a blank name (when present) returns a field error`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)

        val response = client.patch("/api/recipes/${Uuid.random()}") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `PATCH ingredient with a valid payload returns 200`() = testApplication {
        ...
        val response = client.patch("/api/ingredients/$id") {
            contentType(ContentType.Application.Json)
            setBody(payload)
        }
    ...
    @Test
    fun `PATCH ingredient with a blank amount returns a field error and never calls the service`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)

        val response = client.patch("/api/ingredients/${Uuid.random()}") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `PATCH ingredient with a blank name returns a field error`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
    ...
    @Test
    fun `PATCH ingredient with a malformed uuid returns 400`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)

        val response = client.patch("/api/ingredients/not-a-uuid") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `PATCH instruction with a valid payload returns 200`() = testApplication {
        ...
        val response = client.patch("/api/instructions/$id") {
            contentType(ContentType.Application.Json)
            setBody(payload)
        }
    ...
    @Test
    fun `PATCH instruction with a blank description returns a field error and never calls the service`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)

        val response = client.patch("/api/instructions/${Uuid.random()}") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `PATCH recipe notes with a valid body calls the service and returns 204`() = testApplication {
        val service = mockk<RecipeService>()
        val id = Uuid.random()
        coEvery { service.updateRecipeNotes(id, "Great bake!") } returns Unit
        val client = setupTestApp(service)

        val response = client.patch("/api/recipes/notes/$id") {
            contentType(ContentType.Application.Json)
            setBody("Great bake!")
        }
    ...
    @Test
    fun `PATCH recipe notes with a null body clears the notes`() = testApplication {
        val service = mockk<RecipeService>()
        val id = Uuid.random()
    ...
    @Test
    fun `PATCH recipe notes with a malformed uuid returns 400 and never calls the service`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)

        val response = client.patch("/api/recipes/notes/not-a-uuid") {
            contentType(ContentType.Application.Json)
            setBody("Great bake!")
        }
    ...
    @Test
    fun `PATCH ingredient notes with a valid body calls the service and returns 204`() = testApplication {
        val service = mockk<RecipeService>()
        val id = Uuid.random()
        coEvery { service.updateIngredientNotes(id, "Reduce to 480g next time") } returns Unit
        val client = setupTestApp(service)

        val response = client.patch("/api/ingredients/notes/$id") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `PATCH ingredient notes with a malformed uuid returns 400 and never calls the service`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)

        val response = client.patch("/api/ingredients/notes/not-a-uuid") {
            contentType(ContentType.Application.Json)
            setBody("note")
        }
    ...
    @Test
    fun `PATCH instruction notes with a valid body calls the service and returns 204`() = testApplication {
        val service = mockk<RecipeService>()
        val id = Uuid.random()
        coEvery { service.updateInstructionNotes(id, "Rest longer in winter") } returns Unit
        val client = setupTestApp(service)

        val response = client.patch("/api/instructions/notes/$id") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `PATCH instruction notes with a malformed uuid returns 400 and never calls the service`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)

        val response = client.patch("/api/instructions/notes/not-a-uuid") {
            contentType(ContentType.Application.Json)
            setBody("note")
        }
    ...
    @Test
    fun `DELETE recipe with a valid id returns 204 and calls the service`() = testApplication {
        val service = mockk<RecipeService>()
        val id = Uuid.random()
    ...
    @Test
    fun `DELETE recipe with a malformed uuid returns 400 and never calls the service`() = testApplication {
        val service = mockk<RecipeService>()
        val client = setupTestApp(service)
